...
load_config_help[] =
	"\n"
	"For various reasons, one may wish to keep several different " PRODUCT "\n"
	"configurations available on a single machine.\n"
	"\n"
	"If you have saved a previous configuration in a file other than the\n"
	PRODUCT "'s default, entering the name of the file here will allow you\n"
...
save_config_help[] =
	"\n"
	"For various reasons, one may wish to keep different " PRODUCT "\n"
	"configurations available on a single machine.\n"
	"\n"
...
static char filename[PATH_MAX+1] = DEFAULT_FILENAME;

static int indent = 0;
static struct termios ios_org;
static int rows, cols;
/* Rani comment - support for gcc 4.0: static */struct menu *current_menu;
...
int main(int ac, char **av)
{
	...
	sym_calc_value(sym);
	snprintf(menu_backtitle, 128, PRODUCT " v%s Configuration",
	...
	do {
		stat = dialog_yesno(NULL,
				"Do you wish to save your new " PRODUCT " configuration?", 5, 60);
	} while (stat < 0);
	end_dialog();
	...
	if (stat == 0) {
		...
		printf("\n\nYour " PRODUCT " configuration changes were NOT saved.\n\n");
