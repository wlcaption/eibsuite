...
public:
    ...
    JTCCondHelper(JTCRecursiveMutex& mutex, int count)
	: m_mutex(mutex),
	  m_count(count)
    {
    }
...
private:
	...
	//
	// Determine the action, release the internal mutex.
	//
	if (m_to_unblock != 0)
	{
...
//
bool
JTCCond::wait(JTCRecursiveMutex& mutex, long timeout)
{
    if (timeout < 0)
    {
...
//
bool
JTCCond::wait(JTCMutex& mutex, long timeout)
{
    if (timeout < 0)
    {
	throw JTCIllegalArgumentException("timeout value is negative");
...
//
bool
JTCCond::wait_internal(JTCRecursiveMutex& mutex, long timeout)
{
#if defined(HAVE_POSIX_THREADS)
    try
    {
...
//
bool
JTCCond::wait_internal(JTCMutex& mutex, long timeout)
{
#if defined(HAVE_POSIX_THREADS)
...
#if defined(HAVE_POSIX_THREADS)
struct timespec
JTCCond::calc_timeout(long timeout)
{
    assert(timeout >= 0);
    struct timeval tv;
    gettimeofday(&tv, 0);
