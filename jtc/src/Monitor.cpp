...
//
void
JTCMonitorBase::validate_owner(const JTCThreadId& id, const char* caller) const
{
...
{
    //
    // Perform any pending notifies before unlocking the mutex and if
    // the mutex count time is 1.
    //
    if(m_mon_mutex.count() == 1)
    {
	JTCMonitor* This = (JTCMonitor*)this;
	This -> notify_internal(m_nnotify);
    }
...
//
void
JTCMonitor::notify_internal(int nnotify)
{
    //
