...
//
class JTC_IMPORT_EXPORT JTCThread : public virtual JTCRefCount
{
public:
    ...
    //
    static int
    enumerate(JTCThreadHandle* list, int len);
    ...
    //
    static void
    setAttrHook(JTCAttrHook attr_hook, JTCAttrHook* old_hook = 0);
#endif
    ...
    //
    static void
    setRunHook(JTCRunHook run_hook, JTCRunHook* old_hook = 0);
    ...
    //
    static void
    setStartHook(JTCStartHook start_hook, JTCStartHook* old_hook = 0);
...
//
class JTC_IMPORT_EXPORT JTCInitialize
{
    //
